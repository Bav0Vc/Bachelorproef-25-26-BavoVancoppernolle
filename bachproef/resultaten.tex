...
\begin{figure}[!htb]
    ...
    \caption[Source Attribution per embeddingmodel]{Distributie van de Source Attribution-scores
    per embeddingmodel. \texttt{Snowflake} ($\mu = 0{,}372$) en \texttt{E5-large} ($\mu = 0{,}344$)
    zijn statistisch equivalent ($p_{\text{holm}} = 0{,}571$). \texttt{bge-m3} behaalt significant
    de laagste SA ($\mu = 0{,}233$; $p_{\text{holm}} \leq 0{,}047$ t.o.v.\ beide anderen) —
    een inverspatroon ten opzichte van de AR-resultaten (Figuur~\ref{fig:embedder-relevancy}).}
