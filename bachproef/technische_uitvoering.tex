...
De noodzaak voor deze karakter-limieten vloeit voort uit de technische limitaties binnen het \textit{Haystack}-framework. Terwijl de \texttt{RecursiveDocumentSplitter} (gebruikt voor \textit{Fixed-size} en \textit{Recursive Character} splitting) native ondersteuning biedt voor het definiëren van de \texttt{split\_length} in tokens, hanteert de \texttt{EmbeddingBasedDocumentSplitter} (gebruikt voor \textit{Semantic Chunking}) uitsluitend karakters als eenheid voor de \texttt{max\_chunk\_length}~\autocite{HsRds,HsEbds}.
