...
Een bijzonderheid van de implementatie is de \texttt{max\_length}-parameter. Deze wordt in de component als vaste waarde geconfigureerd omdat deze parameter noodzakelijk is om de component te instantiëren, maar wordt later overschreven en dynamisch ingesteld op basis van het geselecteerde \gls{llm} voor die indexeringsrun. Dit wordt verder toegelicht in Sectie~\ref{sec:technische-uitvoering-chunking} over chunk-optimalisatie.
...
def run(self, documents: List[Document]) -> dict:
    ...
    for doc in chunks:
        if len(doc.content or "") > self.max_length:
            ...
            final.extend(
                dataclasses.replace(s, meta={**doc.meta, **s.meta})
                for s in sub
            )
        else:
            final.append(doc)
...
De chunking-deelconfiguratie is de eenvoudigste van de drie: zij slaat uitsluitend de naam van de geselecteerde chunker. De werkelijke chunker-instanties worden in de indexeringspipeline aangemaakt via de \texttt{\_make\_chunker()}-hulpfunctie op basis van deze naam. De drie opties zijn \texttt{RecursiveCharacterSplitter}, \texttt{FixedSizeTokenSplitter} en \texttt{SemanticEmbeddingChunker}.

\subsection{Configuratie embeddingmodellen}

De embedding-deelconfiguratie slaat voor elk model de modelnaam, query-instructieprefix, documentprefix, vectordimensionaliteit en eventuele MRL-truncatiedimensie op. De instructieprefixen zijn modelspecifiek en essentieel voor correcte retrieval.
...
De \gls{llm}-deelconfiguratie koppelt elke modelnaam aan een backend, \gls{api}-parameters en de modelspecifieke \texttt{max\_char\_length}. De achtergrond van deze karakterlimiet wordt toegelicht in Sectie~\ref{sec:technische-uitvoering-chunking}.
...
\end{minted}
\caption[HyPSTER LLM-configuratie]{De \texttt{max\_char\_length}-waarde per \gls{llm} stemt de maximale chunkgrootte van de semantische chunker af op de specifieke tokenizer van het model (Sectie~\ref{sec:technische-uitvoering-chunking}). De twee \texttt{backend}-waarden bepalen welke \gls{api}-client in de querypipeline wordt geïnstantieerd: \texttt{"hf"} voor de HuggingFace Inference Router, \texttt{"mistral"} voor de officiële Mistral \gls{api}.}
...
De indexeringspipeline verwerkt alle brondocumenten voor alle combinaties van chunkingstrategie en embeddingmodel. De deterministische chunkers
— \texttt{RecursiveCharacterSplitter} en \texttt{FixedSizeTokenSplitter} — produceren voor elk van de drie embeddingmodellen één collectie, wat $2 \times 3 = 6$ collecties oplevert.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Fertility ($F$)} & \textbf{Tokens} & \textbf{$C_{target}$ (chars)} \\
\midrule
Llama-3.3-70B-Instruct   & 1,812 & 400 & 1.589 \\
Gemma-3-27B-it           & 1,696 & 400 & 1.698 \\
Mistral-Small-2603       & 1,816 & 400 & 1.586 \\
\bottomrule
\end{tabular}
\caption[Model-specifieke chunk-configuraties voor Italiaanse tekst]{\label{tab:chunk-specs}Model-specifieke chunk-configuraties voor Italiaanse tekstverwerking bij een doelgrootte van 400 tokens.}
\end{table}

Deze \texttt{max\_char\_length}-waarden worden opgeslagen in de HyPSTER \gls{llm}-configuratie (Sectie~\ref{sec:hypster-config}) en doorgegeven aan de \texttt{SemanticEmbeddingChunker} bij de instantiëring in de indexeringspipeline. Voor de twee deterministische chunkers — \texttt{FixedSizeTokenSplitter} en \texttt{RecursiveCharacterSplitter} — is deze conversie niet nodig, omdat zij native in tokens splitsen via de \texttt{RecursiveDocumentSplitter} \autocite{HaystackRecusriveSplitter2026}.
