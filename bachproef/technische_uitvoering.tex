...
De indexeringspipeline verwerkt alle brondocumenten voor alle $3 \times 3 = 9$ combinaties van chunkingstrategie en embeddingmodel. Voor elke combinatie wordt een aparte Qdrant-collectie aangemaakt, deterministisch benoemd als \texttt{<ChunkerNaam>\_<ModelId>}.
...
De \texttt{SemanticEmbeddingChunker} laadt \texttt{paraphrase-multilingual-mpnet-base-v2} op de \gls{gpu} voor de segmentatiefase. Als het primaire embeddingmodel daarna wordt geladen, zijn beide modellen tegelijkertijd in \gls{vram}, wat op \gls{gpu}'s met 8 GB tot een \gls{oom}-fout leidt. De oplossing bestaat uit een tweetraps-uitvoering: na de segmentatiefase wordt het chunkermodel expliciet naar de \gls{cpu} verplaatst en het \gls{cuda}-geheugen gewist:
...
def _free_semantic_chunker_gpu(chunker: SemanticEmbeddingChunker) -> None:
  ...
  if backend is not None:
    ...
    pass
\end{minted}
\caption[\_free\_semantic\_chunker\_gpu: VRAM vrijmaken na segmentatie]{Na de segmentatiefase wordt het interne chunkermodel naar de \gls{cpu} verplaatst. Dit voorkomt \gls{oom}-fouten bij het vervolgens laden van het primaire embeddingmodel, waardoor de semantische chunker-configuraties dezelfde infrastructuur kunnen gebruiken als de deterministische strategieën.}
\end{listing}
