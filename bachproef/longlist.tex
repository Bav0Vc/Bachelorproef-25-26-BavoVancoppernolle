...
\begin{itemize}
  ...
  \item \textbf{EU-Soevereiniteit (RE-04):} Dit criterium is belangrijk betreffende \gls{gdpr}-compliance en databeveiliging. Er wordt gekeken of het model lokaal gehost kan worden of beheerd wordt door een Europese entiteit, waardoor gevoelige bedrijfsgegevens de \gls{eu} niet verlaten.
\end{itemize}
...
\bottomrule
\end{tabular}
\caption[Longlist: vergelijkingstabel embeddingmodellen]{\label{tab:embeddingmodels}Vergelijking van embeddingmodellen op een ordinale schaal (1-5).}
\vspace{2mm}
...
\begin{itemize}
  \item \textbf{Parameters/Modelomvang (RL-01):} De grootte van het model beïnvloedt de redeneerkracht. Grotere modellen (zoals 70B+) kunnen complexere \gls{esg}-vragen beantwoorden, maar vereisen meer geheugen en genereren antwoorden vaak trager.
  \item \textbf{Instructie-tuned (RL-02):} Geeft aan of het model specifiek is getraind om opdrachten uit te voeren. Zonder deze eigenschap vult een model tekst aan in plaats van een vraag te beantwoorden, wat de \gls{rag}-architectuur ondermijnt.
